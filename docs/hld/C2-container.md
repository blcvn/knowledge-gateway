...
C4Container
    ...
    System_Boundary(vnp, "VNP Memory — Single Binary (backend/apps/memory)") {
        ...
        Container(gateway, "API Gateway", "Go — net/http",
            "Entry point: REST :8080, MCP :8082, Health :8083\nAuth middleware, rate limiting, routing\nInProcessRegistry lookup")

        Container(mcp, "MCP Server", "Go — JSON-RPC 2.0",
            "37+ tools: memory_store, memory_recall,\nov_grep, ov_read_file, ov_search...\nSSE transport + HTTP Streamable")
        ...
        Container(agent_memory, "AgentMemory Layer (4)", "Go — gRPC in-process",
            "observe-service (12 hooks, 14-step pipeline)\nmemory-service (lifecycle, versioning)\norchestration-service (leases, signals, DAG)\npipeline-service (4-tier consolidation)")

        Container(platform, "Platform Services", "Go — gRPC in-process",
            "vnp-platform (admin, tenants, API keys)\nvnp-event (timeline, UserEvent)\nvnp-observability (metrics, traces)\nvnp-dashboard (Console UI embed)")
    ...
    Rel(gateway, agent_memory, "Observe hooks\nOrchestration calls", "bufconn gRPC")
    Rel(gateway, platform, "Admin, auth, events", "bufconn gRPC")
    Rel(mcp, gateway, "Translate MCP tools\nto REST/gRPC calls", "in-process")
...
│                                                                     │
│  ┌──────────────┐                                                   │
│  │  API Gateway │ REST :8080 / MCP :8082 / Health :8083            │
...
│  │  MinIO (distributed)      │ Qdrant (optional)                    │
│  └───────────────────────────┘                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
|---|---|---|---|
| API Gateway | Go net/http | Auth, routing, rate limiting | :8080, :8082, :8083 |
| MCP Server | Go JSON-RPC 2.0 | 37+ AI assistant tools | :8082 (SSE + HTTP) |
...
| AgentMemory Layer (×4) | Go gRPC | Observe, lifecycle, orchestration, consolidation | in-process / network |
| Platform Services | Go gRPC | Admin, events, observability, UI | in-process / network |
...
| Qdrant | Vector DB | Optional high-scale vector search | 6333 |
