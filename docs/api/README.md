...
| `POST` | `/v1/kg/read/template/{domain_id}/{template_name}` | `200` | Generic dynamic route for active templates only. |
| `GET` | `/v1/kg/read/nodes/{id}` | `200` | Returns visible node details and relationship IDs. |
| `POST` | `/v1/kg/search/semantic` | `200` | Uses `query`, `domain_ids`, `top_k`. |
...
| Method | Path | Success | Notes |
| --- | --- | --- | --- |
| `POST` | `/v1/kg/write/nodes` | `202` | Async acknowledgment with `node_id`, `domain_version`, `status`, `sync_eta_ms`. |
...
| `POST` | `/v1/kg/write/relationships` | `201` | Creates relationship and returns `relationship_id`. |
