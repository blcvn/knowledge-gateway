// Package embed provides startup functions for embedding memobase microservices
...
// service startup pattern used by all memobase microservices.
//
// This pattern is identical across memobase-ingestion, memobase-engine,
// memobase-context, and memobase-pipeline because they all follow the
// same enterprise bootstrap template.
//
// Reference: services/memobase-*/cmd/server/main.go
func StartGRPCService(ctx context.Context, serviceName string, grpcPort int) error {
	...
	healthPort := grpcPort + 100 // e.g., 9141 for ingestion (port 9041)
	go func() {
