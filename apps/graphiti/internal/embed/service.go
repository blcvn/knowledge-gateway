// Package embed provides startup functions for embedding graphiti microservices
...
// service startup pattern used by all graphiti microservices.
//
// This pattern is identical across graphiti-store, graphiti-knowledge,
// graphiti-ingestion, graphiti-search, and graphiti-pipeline because they
// all follow the same enterprise bootstrap template.
//
// Reference: services/graphiti-*/cmd/server/main.go
func StartGRPCService(ctx context.Context, serviceName string, grpcPort int) error {
	...
	healthPort := grpcPort + 100 // e.g., 9121 for ingestion (port 9021)
	go func() {
