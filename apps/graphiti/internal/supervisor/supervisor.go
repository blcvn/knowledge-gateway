...
// goroutines within the graphiti monolith process.
//
...
//
// Phase ordering ensures that downstream dependencies (e.g., Neo4j via store)
// are ready before upstream services (e.g., ingestion, search) attempt to
// connect.
package supervisor
...
const (
	// PhaseData — Store service starts first (Neo4j connections).
	PhaseData Phase = 0
	// PhaseIntelligence — Knowledge service starts after store is ready.
	PhaseIntelligence Phase = 1
	// PhaseApplication — Ingestion, search, pipeline start after knowledge is ready.
...
type ServiceSpec struct {
	Name    string                          // Human-readable service name (e.g. "graphiti-store")
...
//  1. Phase 0 (data): graphiti-store
//  2. Phase 1 (intelligence): graphiti-knowledge
//  3. Phase 2 (application): graphiti-ingestion, graphiti-search, graphiti-pipeline
