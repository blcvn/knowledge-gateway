...
- `gateway/adapter/mcp/server.go` — MCP SSE + HTTP Streamable server (37+ tools target)
...
**22 tools currently registered** (target: 37+ per HLD; source: `gateway/adapter/mcp/server.go`):
