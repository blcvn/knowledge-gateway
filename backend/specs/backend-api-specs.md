...
> **Version**: Derived from source code as of 2026-06-18  
> **Source of truth**: `gateway/internal/adapter/handler/router.go` + handler files  
> **Protocols**: REST/HTTP (8080), gRPC (8081), MCP (8082), WebDAV (8080/webdav)
...
> **Note**: A dedicated `/v1/auth/*` login/logout/refresh API is **not yet implemented** in the gateway router. This is a known gap (see [CR-001](../crs/v2/api-update/CR-001-frontend-api-alignment.md)).
...
| `type` field | Target Service |
|---|---|
| `semantic` | `cognee-ingestion` |
| `episodic` | `graphiti-ingestion` |
| `conversational` | `memobase-ingestion` |
| `profile` | `memobase-ingestion` |
| `procedural` | `ov-resource` |
| `auto` | LLM classifier → re-route |

### Agent Memory Management — `/v1/memory/agent/*`
| Method | Path | Backend Service | Description |
|--------|------|----------------|-------------|
| `POST` | `/v1/memory/agent/remember` | `memory-service` | Store agent memory |
| `GET` | `/v1/memory/agent/list` | `memory-service` | List agent memories |
| `GET` | `/v1/memory/agent/{id}` | `memory-service` | Get agent memory |
| `DELETE` | `/v1/memory/agent/{id}` | `memory-service` | Delete agent memory |
| `GET` | `/v1/memory/agent/{id}/retention` | `memory-service` | Get retention score |
| `POST` | `/v1/memory/agent/evict` | `memory-service` | Evict memories manually |
| `POST` | `/v1/memory/agent/auto-forget` | `memory-service` | Trigger auto-forget sweep |
| `DELETE` | `/v1/memory/agent/{id}/governance` | `memory-service` | Delete for governance |
| `GET` | `/v1/memory/audit` | `memory-service` | List audit logs |

### Memory Slots — `/v1/memory/slots/*`
| Method | Path | Backend Service | Description |
|--------|------|----------------|-------------|
| `GET` | `/v1/memory/slots` | `memory-service` | List all memory slots |
| `GET` | `/v1/memory/slots/{scope}/{label}` | `memory-service` | Get a specific slot |
| `POST` | `/v1/memory/slots/{scope}/{label}` | `memory-service` | Write to a memory slot |
| `DELETE` | `/v1/memory/slots/{scope}/{label}` | `memory-service` | Delete a memory slot |

### Consolidation & Learnings
| Method | Path | Backend Service | Description |
|--------|------|----------------|-------------|
| `POST` | `/v1/memory/compress` | `consolidation-service` | Compress observations |
| `POST` | `/v1/memory/summarize` | `consolidation-service` | Summarize session |
| `POST` | `/v1/memory/consolidate` | `consolidation-service` | Run consolidation pipeline |
| `GET` | `/v1/memory/procedural` | `consolidation-service` | List procedural memories |
| `GET` | `/v1/memory/procedural/{id}` | `consolidation-service` | Get procedural memory |
| `GET` | `/v1/memory/lessons` | `consolidation-service` | List lessons |
| `GET` | `/v1/memory/lessons/{id}` | `consolidation-service` | Get lesson |
| `POST` | `/v1/memory/lessons/decay-sweep` | `consolidation-service` | Execute lesson decay sweep |
| `GET` | `/v1/memory/insights` | `consolidation-service` | List insights |
