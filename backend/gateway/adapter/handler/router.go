...
func Router(
	...
	// === /v1/auth/* (Authentication & SSO) — TASK-BE-002 ===
	if auth != nil {
		// Public routes (không cần auth middleware)
		...
		mux.HandleFunc("POST /v1/auth/sso/google", auth.LoginWithGoogle)
	...
	// CR-COGNEE-001: Non-destructive graph enrichment (Memify)
	mux.HandleFunc("POST /v1/cognee/datasets/{id}/memify", cognee.Memify)
	mux.HandleFunc("GET /v1/cognee/datasets/{id}/memify/status", cognee.GetMemifyStatus)
	// CR-COGNEE-002: NodeSets scoping is handled transparently in the Search payload
	// CR-COGNEE-003: Schema-defined DataPoint ingestion (zero LLM tokens)
	mux.HandleFunc("POST /v1/cognee/datasets/{id}/datapoints", cognee.AddDataPoints)
