...
func setupTestServer(t *testing.T) (*httptest.Server, *mockRegistry) {
	...
	orgH := handler.NewOrgHandler(reg, logger)
	sdkH := handler.NewSDKHandler(reg, logger)
	agentmemH := handler.NewAgentMemoryHandler(reg, logger)

	router := handler.Router(
		memoryH, cogneeH, graphitiH, memobaseH, ovH, zepH, smH, adminH, nil, // auth=nil for tests
