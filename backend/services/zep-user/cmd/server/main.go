...
// Enterprise-grade bootstrap for zep-user
func main() {
	...
	slog.Info("Initializing zep-user at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "zep-user")
	if err != nil {
	...
	healthCheck.SetServingStatus("zep-user", grpc_health_v1.HealthCheckResponse_SERVING)
