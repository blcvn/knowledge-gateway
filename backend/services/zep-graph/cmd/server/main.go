...
// Enterprise-grade bootstrap for zep-graph
func main() {
	...
	slog.Info("Initializing zep-graph at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "zep-graph")
	if err != nil {
	...
	healthCheck.SetServingStatus("zep-graph", grpc_health_v1.HealthCheckResponse_SERVING)
