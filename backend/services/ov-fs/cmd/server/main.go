...
// Enterprise-grade bootstrap for ov-fs
func main() {
	...
	slog.Info("Initializing ov-fs at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "ov-fs")
	if err != nil {
	...
	healthCheck.SetServingStatus("ov-fs", grpc_health_v1.HealthCheckResponse_SERVING)
