...
// Enterprise-grade bootstrap for graphiti-search
func main() {
	...
	slog.Info("Initializing graphiti-search at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "graphiti-search")
	if err != nil {
	...
	healthCheck.SetServingStatus("graphiti-search", grpc_health_v1.HealthCheckResponse_SERVING)
