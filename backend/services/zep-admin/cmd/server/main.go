...
// Enterprise-grade bootstrap for zep-admin
func main() {
	...
	slog.Info("Initializing zep-admin at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "zep-admin")
	if err != nil {
	...
	healthCheck.SetServingStatus("zep-admin", grpc_health_v1.HealthCheckResponse_SERVING)
