...
func (s *ExtractRelationshipsStage) Execute(ctx context.Context, job *domain.CognifyJob, state *CognifyPipelineState) error {
	...
	for _, chunk := range state.Chunks {
		...
		userPrompt := fmt.Sprintf("Entities in context: %s\n\nText:\n%s",
			strings.Join(entityNames, ", "), chunk.Content)
