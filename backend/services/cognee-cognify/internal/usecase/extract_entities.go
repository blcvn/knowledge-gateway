...
func (s *ExtractEntitiesStage) Execute(ctx context.Context, job *domain.CognifyJob, state *CognifyPipelineState) error {
	var allEntities []*domain.Entity

	for _, chunk := range state.Chunks {
		resp := &llmEntityResponse{}
		err := s.llm.CompleteStructured(ctx, entityExtractionPrompt, chunk.Content, resp)
		if err != nil {
