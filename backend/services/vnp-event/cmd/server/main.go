...
// Enterprise-grade bootstrap for vnp-event
func main() {
	...
	slog.Info("Initializing vnp-event at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "vnp-event")
	if err != nil {
	...
	healthCheck.SetServingStatus("vnp-event", grpc_health_v1.HealthCheckResponse_SERVING)
