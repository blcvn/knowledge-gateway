...
//
// Covers: TenantRepo, EventRepo, UsageRepo, SpaceRepo
// (aligned with actual domain entity fields)
package persistence
...
func (r *TenantRepo) Create(ctx context.Context, t *admin.Tenant) error {
	...
	_, err := r.pool.Exec(ctx,
		...
		t.ID, t.Name, t.Slug, t.Tier, metaJSON, t.CreatedAt, t.UpdatedAt,
	)
	return err
}
...
func (r *TenantRepo) FindByID(ctx context.Context, id uuid.UUID) (*admin.Tenant, error) {
	...
	).Scan(&t.ID, &t.Name, &t.Slug, &t.Tier, &metaJSON, &active, &t.CreatedAt, &t.UpdatedAt)
	...
	if !active {
		t.Status = admin.TenantSuspended
	} else {
		t.Status = admin.TenantActive
	}
	return t, nil
}
...
func (r *TenantRepo) Update(ctx context.Context, t *admin.Tenant) error {
	metaJSON, _ := json.Marshal(t.Metadata)
	active := t.Status == admin.TenantActive
	_, err := r.pool.Exec(ctx,
		`UPDATE tenants SET name=$1, slug=$2, tier=$3, config=$4, active=$5, updated_at=$6 WHERE id=$7`,
		t.Name, t.Slug, t.Tier, metaJSON, active, t.UpdatedAt, t.ID,
	)
	return err
}
...
func (r *TenantRepo) List(ctx context.Context, offset, limit int) ([]*admin.Tenant, int, error) {
	...
	for rows.Next() {
		...
		var metaJSON []byte
		var active bool
		if err := rows.Scan(&t.ID, &t.Name, &t.Slug, &t.Tier, &metaJSON, &active, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, 0, err
		}
		_ = json.Unmarshal(metaJSON, &t.Metadata)
		if active {
...
func (r *EventRepo) Create(ctx context.Context, evt *event.UserEvent) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO user_events (id, user_id, tenant_id, source, content, tags, created_at, valid_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		evt.ID, evt.UserID, evt.TenantID,
		string(evt.Engine), evt.GistText, // source=engine, content=gistText
		[]string{string(evt.Type)}, evt.CreatedAt, evt.CreatedAt,
	)
	return err
}
...
func (r *EventRepo) FindByUser(ctx context.Context, tenantID, userID uuid.UUID, limit int) ([]event.UserEvent, int, error) {
	...
	rows, err := r.pool.Query(ctx,
		`SELECT id, user_id, tenant_id, source, content, tags, created_at
	...
	for rows.Next() {
		...
		var tags []string
		if err := rows.Scan(&e.ID, &e.UserID, &e.TenantID, &source, &content, &tags, &e.CreatedAt); err != nil {
			return nil, 0, err
		}
		e.Engine = source
		e.GistText = content
		if len(tags) > 0 {
			e.Type = event.EventType(tags[0])
		}
	...
	_, err := r.pool.Exec(ctx,
		`INSERT INTO usage_records (id, tenant_id, period, api_calls, storage_mb, llm_tokens, updated_at)
		 ...
		 ON CONFLICT (tenant_id, period) DO UPDATE SET
		   ...
		   updated_at = NOW()`,
		rec.ID, rec.TenantID, rec.Period, rec.Requests, 0, rec.Tokens,
	)
	return err
}
...
func (r *UsageRepo) FindByTenant(ctx context.Context, tenantID uuid.UUID, period string) ([]*analytics.UsageRecord, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, tenant_id, period, api_calls, llm_tokens, created_at
	...
	for rows.Next() {
		rec := &analytics.UsageRecord{TenantID: tenantID, Period: period}
		var storageMB int64
		if err := rows.Scan(&rec.ID, &rec.TenantID, &rec.Period, &rec.Requests,
			&rec.Tokens, &rec.Date); err != nil {
			_ = storageMB
			return nil, err
		}
