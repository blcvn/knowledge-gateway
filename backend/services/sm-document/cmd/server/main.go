...
// Enterprise-grade bootstrap for sm-document
func main() {
	...
	slog.Info("Initializing sm-document at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "sm-document")
	if err != nil {
	...
	healthCheck.SetServingStatus("sm-document", grpc_health_v1.HealthCheckResponse_SERVING)
