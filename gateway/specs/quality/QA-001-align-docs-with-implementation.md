...
The gateway has a production-grade Go implementation in `gateway/`. All docs and specs live alongside the implementation in this directory.
...
- [ ] AC-1: Every endpoint in `gateway/internal/adapter/handler/router.go` is documented in `gateway/docs/api.md`
- [ ] AC-2: Every env var in `gateway/internal/infra/config/config.go` is documented in `gateway/docs/configuration.md`
- [ ] AC-3: `gateway/specs/tdd.md` file paths match actual gateway implementation
