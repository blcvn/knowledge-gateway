...
Bridge and sync tooling for the local CodeGraph index and the current `kg-service` API surface.
...
The repository root contains executable wrappers:
...
`codegraph-sync/sync --full` performs a full reindex by deleting the project nodes tracked in the
local manifest before the new batch is written.

## Environment

Use `codegraph-sync/.env.example` as the starting point.
