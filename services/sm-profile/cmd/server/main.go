...
// Enterprise-grade bootstrap for sm-profile
func main() {
	...
	slog.Info("Initializing sm-profile at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "sm-profile")
	if err != nil {
	...
	healthCheck.SetServingStatus("sm-profile", grpc_health_v1.HealthCheckResponse_SERVING)
