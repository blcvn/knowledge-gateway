...
// Enterprise-grade bootstrap for ov-session
func main() {
	...
	slog.Info("Initializing ov-session at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "ov-session")
	if err != nil {
	...
	healthCheck.SetServingStatus("ov-session", grpc_health_v1.HealthCheckResponse_SERVING)
