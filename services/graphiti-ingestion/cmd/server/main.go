...
// Enterprise-grade bootstrap for graphiti-ingestion
func main() {
	...
	slog.Info("Initializing graphiti-ingestion at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "graphiti-ingestion")
	if err != nil {
	...
	healthCheck.SetServingStatus("graphiti-ingestion", grpc_health_v1.HealthCheckResponse_SERVING)
