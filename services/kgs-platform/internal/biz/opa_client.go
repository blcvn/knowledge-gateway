...
// URL is read from config (e.g. http://opa:8181) not hardcoded to localhost
func NewOPAClient(conf *conf.Data, logger log.Logger) *OPAClient {
	opaURL := "http://localhost:8181"
	if conf != nil && conf.Opa != nil && conf.Opa.Url != "" {
		opaURL = conf.Opa.Url
	}
	return &OPAClient{
		baseURL: opaURL + "/v1/data/kgs/allow",
