...
// Enterprise-grade bootstrap for ba-knowledge-worker
func main() {
	...
	slog.Info("Initializing ba-knowledge-worker at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "ba-knowledge-worker")
	if err != nil {
	...
	healthCheck.SetServingStatus("ba-knowledge-worker", grpc_health_v1.HealthCheckResponse_SERVING)
