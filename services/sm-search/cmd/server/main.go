...
// Enterprise-grade bootstrap for sm-search
func main() {
	...
	slog.Info("Initializing sm-search at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "sm-search")
	if err != nil {
	...
	healthCheck.SetServingStatus("sm-search", grpc_health_v1.HealthCheckResponse_SERVING)
