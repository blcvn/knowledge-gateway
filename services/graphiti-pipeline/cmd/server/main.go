...
// Enterprise-grade bootstrap for graphiti-pipeline
func main() {
	...
	slog.Info("Initializing graphiti-pipeline at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "graphiti-pipeline")
	if err != nil {
	...
	healthCheck.SetServingStatus("graphiti-pipeline", grpc_health_v1.HealthCheckResponse_SERVING)
