...
// Enterprise-grade bootstrap for zep-search
func main() {
	...
	slog.Info("Initializing zep-search at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "zep-search")
	if err != nil {
	...
	healthCheck.SetServingStatus("zep-search", grpc_health_v1.HealthCheckResponse_SERVING)
