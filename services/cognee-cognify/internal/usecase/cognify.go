...
func (uc *CognifyOrchestrator) Execute(ctx context.Context, req dto.TriggerCognifyRequest) (*dto.CognifyJobResult, error) {
	...
	// Create job
	job := domain.NewCognifyJob(req.TenantID, req.DatasetID, cfg)
	...
	state := &PipelineState{TextItems: textItems}
	...
	for i, stage := range uc.stages {
		...
		// Advance job stage
		progress := domain.StageProgress(i)
		job.AdvanceStage(allStages[i], progress)
		_ = uc.jobRepo.Update(ctx, job)
