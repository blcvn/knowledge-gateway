...
func (s *DeduplicateStage) Execute(ctx context.Context, job *domain.CognifyJob, state *PipelineState) error {
	...
	for _, entity := range state.Entities {
		...
		if canonical, exists := seen[key]; exists {
			mergeMap[entity.ID.String()] = canonical.ID.String()
			dedupCount++
		} else {
			seen[key] = entity
	...
	if len(mergeMap) > 0 {
		for _, rel := range state.Relationships {
			if newID, ok := mergeMap[rel.SourceID.String()]; ok {
				if canonical, found := findEntityByIDStr(deduplicated, newID); found {
					rel.SourceID = canonical.ID
				}
			}
			if newID, ok := mergeMap[rel.TargetID.String()]; ok {
				if canonical, found := findEntityByIDStr(deduplicated, newID); found {
