...
// Stage is the interface for each pipeline stage.
type Stage interface {
	Name() domain.StageType
	Execute(ctx context.Context, job *domain.CognifyJob, state *PipelineState) error
}

// PipelineState holds intermediate results passed between stages.
...
func (s *ClassifyStage) Execute(ctx context.Context, job *domain.CognifyJob, state *PipelineState) error {
