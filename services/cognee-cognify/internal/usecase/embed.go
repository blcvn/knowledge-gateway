...
func (s *EmbedStage) Execute(ctx context.Context, job *domain.CognifyJob, state *PipelineState) error {
	embeddingsCount := 0
	...
	for i := 0; i < len(state.Chunks); i += batchSize {
		...
		for j, chunk := range batch {
			...
			if err := s.vectorRepo.UpsertChunkEmbedding(ctx, job.TenantID, chunk.ID, chunk.Text, embeddings[j]); err != nil {
	...
	for _, entity := range state.Entities {
		...
		if err := s.vectorRepo.UpsertEntityEmbedding(ctx, job.TenantID, entity.ID, text, embedding); err != nil {
