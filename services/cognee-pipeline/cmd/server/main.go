...
// Enterprise-grade bootstrap for cognee-pipeline
func main() {
	...
	slog.Info("Initializing cognee-pipeline at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "cognee-pipeline")
	if err != nil {
	...
	healthCheck.SetServingStatus("cognee-pipeline", grpc_health_v1.HealthCheckResponse_SERVING)
