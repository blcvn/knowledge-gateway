...
// Enterprise-grade bootstrap for zep-memory
func main() {
	...
	slog.Info("Initializing zep-memory at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "zep-memory")
	if err != nil {
	...
	healthCheck.SetServingStatus("zep-memory", grpc_health_v1.HealthCheckResponse_SERVING)
