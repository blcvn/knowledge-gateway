...
// Enterprise-grade bootstrap for zep-thread
func main() {
	...
	slog.Info("Initializing zep-thread at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "zep-thread")
	if err != nil {
	...
	healthCheck.SetServingStatus("zep-thread", grpc_health_v1.HealthCheckResponse_SERVING)
