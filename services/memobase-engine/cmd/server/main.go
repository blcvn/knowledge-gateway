...
// Enterprise-grade bootstrap for memobase-engine
func main() {
	...
	slog.Info("Initializing memobase-engine at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "memobase-engine")
	if err != nil {
	...
	healthCheck.SetServingStatus("memobase-engine", grpc_health_v1.HealthCheckResponse_SERVING)
