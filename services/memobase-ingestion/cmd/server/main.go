...
// Enterprise-grade bootstrap for memobase-ingestion
func main() {
	...
	slog.Info("Initializing memobase-ingestion at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "memobase-ingestion")
	if err != nil {
	...
	healthCheck.SetServingStatus("memobase-ingestion", grpc_health_v1.HealthCheckResponse_SERVING)
