...
// Enterprise-grade bootstrap for sm-analytics
func main() {
	...
	slog.Info("Initializing sm-analytics at enterprise-grade...")

	// 2. Initialize OpenTelemetry Distributed Tracing
	shutdownTracer, err := telemetry.InitProvider(ctx, "sm-analytics")
	if err != nil {
	...
	healthCheck.SetServingStatus("sm-analytics", grpc_health_v1.HealthCheckResponse_SERVING)
