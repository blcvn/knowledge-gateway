...
-- Migration: 0042_orchestration.up.sql
...
CREATE INDEX IF NOT EXISTS idx_actions_tenant_status ON actions(tenant_id, status, project);
CREATE INDEX IF NOT EXISTS idx_leases_action ON leases(action_id, status) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_leases_expires ON leases(expires_at) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_signals_to_agent ON signals(tenant_id, to_agent, is_read);
CREATE INDEX IF NOT EXISTS idx_signals_expires ON signals(expires_at);
CREATE INDEX IF NOT EXISTS idx_checkpoints_status ON checkpoints(tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_sentinels_watching ON sentinels(status) WHERE status = 'watching';
