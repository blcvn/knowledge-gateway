...
export KG_RUNTIME_STATE_FILE="${KG_RUNTIME_STATE_FILE:-${repo_root}/codegraph-sync/.state/codegraph-runtime-bootstrap.json}"
...
if [[ "${skip_compose}" -eq 0 ]]; then
  ...
  "${repo_root}/scripts/deploy-compose-codegraph-runtime.sh"
else
...
if [[ "${skip_ontology_bootstrap}" -eq 0 ]]; then
  ...
  "${repo_root}/scripts/bootstrap-codegraph-ontology.sh"
else
...
if [[ "${skip_verify}" -eq 0 ]]; then
  ...
  "${repo_root}/scripts/verify-codegraph-ontology.sh"
else
...
if [[ "${skip_sync}" -eq 0 ]]; then
  ...
  "${repo_root}/codegraph-sync/sync:dry"

  step "upsert CodeGraph index"
  "${repo_root}/codegraph-sync/sync"
else
...
PY
    ...
    "${repo_root}/scripts/codegraph-refresh.sh"
    "${repo_root}/codegraph-sync/sync"
...
created = request(
    ...
    "/v1/kg/write/nodes",
    {
        ...
        "properties": {
            "name": node_name,
            "kind": "function",
            "file": "scripts/validate-codegraph-runtime.sh",
            "line": 1,
            "language": "bash",
