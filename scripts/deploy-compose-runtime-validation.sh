...
compose_file="${repo_root}/deploy/compose/runtime-validation/docker-compose.yml"

# shellcheck source=runtime-profile.sh
source "${repo_root}/scripts/runtime-profile.sh"

if [[ -z "${KG_RUNTIME_PROFILE:-}" ]]; then
  echo "KG_RUNTIME_PROFILE is required for the runtime-validation Compose path" >&2
  exit 1
fi
...
echo "starting runtime-validation Compose stack from ${compose_file}"
